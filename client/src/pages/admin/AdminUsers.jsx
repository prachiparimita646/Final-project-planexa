...
import {
  Search, Shield, Edit2, Trash2, X, Save, Calendar, Users
} from "lucide-react";
  ...
  return (
    <div style={{ fontFamily:"'Jost', sans-serif", backgroundColor:BG, minHeight:"100vh", color:"#2c1a0e", padding:"24px 28px 48px", boxSizing:"border-box" }}>
      <style>{`
        ...
        .au-card:hover { transform:translateY(-3px); box-shadow:0 10px 28px rgba(139,94,60,0.14)!important; }
      ...
      {/* Summary */}
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(350px,1fr))", gap:14, marginBottom:24, animation:"fadeUp 0.4s ease" }}>
        {[
          ...
          <div key={i} style={{ background:CARD, borderRadius:14, padding:"16px 50px", border:`1px solid ${BORDER}`, boxShadow:SHADOW, position:"relative", overflow:"hidden" }}>
      ...
      {/* Filters */}
      <div style={{ background:CARD, borderRadius:14, padding:"14px 18px", border:`1px solid ${BORDER}`, marginBottom:18, display:"flex", gap:12, flexWrap:"wrap", alignItems:"center" }}>
        ...
        <div style={{ display:"flex", gap:8 }}>
          {["all","user","admin"].map(f=>(
            <button key={f} onClick={()=>setFilterRole(f)}
              style={{ padding:"7px 14px", borderRadius:9, border:`1px solid ${BORDER}`, background:filterRole===f?"#8b5e3c":BG, color:filterRole===f?"#fff":"#6b4c35", fontWeight:700, fontSize:"0.76rem", cursor:"pointer", textTransform:"capitalize", fontFamily:"inherit" }}>
      ...
      {!loading && (
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))", gap:16 }}>
          {filtered.map((u,idx)=>{
            ...
            return (
              <div key={u._id} className="au-card" style={{ background:CARD, borderRadius:16, border:`1px solid ${BORDER}`, boxShadow:SHADOW, overflow:"hidden" }}>
                ...
                <div style={{ padding:"18px 20px" }}>
                  <div style={{ display:"flex", alignItems:"center", gap:13, marginBottom:14 }}>
                    ...
                    <div style={{ flex:1, minWidth:0 }}>
                      <div style={{ display:"flex", alignItems:"center", gap:7, flexWrap:"wrap", marginBottom:4 }}>
                        ...
                        {u.role==="admin"&&(
                          <span style={{ display:"inline-flex", alignItems:"center", gap:3, background:"#f0e8ff", color:"#6b2fa0", fontSize:"0.6rem", fontWeight:700, padding:"2px 7px", borderRadius:999 }}>
                            <Shield size={9}/> Admin
                          </span>
                        )}
                      </div>
                  ...
                  <div style={{ display:"flex", gap:8, paddingTop:12, borderTop:`1px solid ${BORDER}` }}>
                    <button className="au-btn" onClick={()=>{setEditUser(u);setEditForm({name:u.name,email:u.email,role:u.role});}}
                      style={{ flex:1, padding:"8px", borderRadius:9, background:BG, border:`1px solid ${BORDER}`, color:"#8b5e3c", fontWeight:700, fontSize:"0.77rem", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:5, fontFamily:"inherit" }}>
                      <Edit2 size={12}/> Edit
                    </button>
                    <button className="au-btn" onClick={()=>toggleRole(u._id,u.role,u.name)} title={u.role==="user"?"Promote to Admin":"Demote to User"}
                      style={{ width:34, height:34, borderRadius:9, background:u.role==="admin"?"#f0e8ff":CARD2, border:"none", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", color:u.role==="admin"?"#6b2fa0":"#a88972" }}>
                      <Shield size={14}/>
                    </button>
                    <button className="au-btn" onClick={()=>deleteUser(u._id,u.name)}
                      style={{ width:34, height:34, borderRadius:9, background:"#f6dddd", border:"none", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", color:"#8b3a2a" }}>
                      <Trash2 size={14}/>
                    </button>
                  </div>
                </div>
      ...
      {editUser&&(
        <div onClick={()=>setEditUser(null)} style={{ position:"fixed", inset:0, background:"rgba(44,26,14,0.5)", zIndex:1000, display:"flex", alignItems:"center", justifyContent:"center", padding:24 }}>
          <div onClick={e=>e.stopPropagation()} style={{ background:BG, borderRadius:22, width:"100%", maxWidth:440, boxShadow:"0 24px 64px rgba(44,26,14,0.22)", overflow:"hidden" }}>
            ...
            <div style={{ padding:"22px 24px", display:"flex", flexDirection:"column", gap:14 }}>
              {[
                ...
                <div key={f.key}>
                  ...
                  <input type={f.type} value={editForm[f.key]||""} onChange={e=>setEditForm(p=>({...p,[f.key]:e.target.value}))}
                    style={{ width:"100%", padding:"9px 13px", borderRadius:9, border:`1px solid ${BORDER}`, fontSize:"0.86rem", color:"#2c1a0e", background:CARD, outline:"none", fontFamily:"inherit", boxSizing:"border-box" }} />
                </div>
              ))}
              <div>
                ...
                <select value={editForm.role||"user"} onChange={e=>setEditForm(p=>({...p,role:e.target.value}))}
                  style={{ width:"100%", padding:"9px 13px", borderRadius:9, border:`1px solid ${BORDER}`, fontSize:"0.86rem", color:"#2c1a0e", background:CARD, outline:"none", fontFamily:"inherit", cursor:"pointer" }}>
              ...
              <div style={{ display:"flex", gap:10, marginTop:4 }}>
                ...
                <button onClick={()=>setEditUser(null)} style={{ padding:"11px 20px", borderRadius:11, background:CARD2, color:"#6b4c35", fontWeight:700, fontSize:"0.88rem", border:`1px solid ${BORDER}`, cursor:"pointer", fontFamily:"inherit" }}>
                  Cancel
                </button>
              </div>
            </div>
